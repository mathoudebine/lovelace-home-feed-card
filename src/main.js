 ...
 <ha-icon icon="mdi:calendar"></ha-icon> ${calendarObject.attributes["friendly_name"]}
  ...
  _handleClick(ev) {
    ...
    _renderItem(n) {
        ...
        switch(n.item_type)
        {
            case "notification":
                ...
                if(message.charAt(0) === "{")
                {
                    ...
                    var param = message.substring(message.indexOf("{"), message.indexOf("}")+1);

                    if(JSON.parse(param).icon !== undefined)
                    {
                        icon = JSON.parse(param).icon;
                    }
                    if(JSON.parse(param).severity !== undefined)
                    {
                        severity = JSON.parse(param).severity;
                    }
        ...
        let iconShowOrHide = !show_icons ? "display:none;" : "";
        let iconClass = this._config.rtl === true ? "item-left rtl" : "item-left";

        let itemCustomColor = "";
        let iconStateColor = this._config.state_color;
        if(severity !== undefined)
        {
            iconStateColor = false;

            if(severity === "warn" || severity === "warning")
            {
                itemCustomColor = "background-color:var(--warning-color); color:var(--text-primary-color);"
            }
            else if(severity === "error")
            {
                itemCustomColor = "background-color:var(--error-color); color:var(--text-primary-color);"
            }
            else if(severity === "success")
            {
                itemCustomColor = "background-color:var(--success-color); color:var(--text-primary-color);"
            }
            else if(severity === "info")
            {
                itemCustomColor = "background-color:var(--info-color); color:var(--text-primary-color);"
            }
        }
        return html`
        <div class="item-container" style="${itemCustomColor}">
            <div class="${iconClass}" style="${iconShowOrHide}">
                <state-badge style="${itemCustomColor}" .stateColor=${iconStateColor} .stateObj='${stateObj}' .overrideIcon='${icon}'/>
            </div>
        ...
        ${compact_mode ? html`` : html`<hr style="clear:both;color:var(--card-background-color);"/>`}
        `;
    }
